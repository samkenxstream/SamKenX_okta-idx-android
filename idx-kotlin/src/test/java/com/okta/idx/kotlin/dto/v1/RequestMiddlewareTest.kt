...
class RequestMiddlewareTest {
    ...
    @Test fun testInteractContextWithExtraParameters() {
        val extraParameters = mapOf(Pair("activation_token", "secret"))
        ...
        val buffer = Buffer()
        request.body?.writeTo(buffer)
        assertThat(buffer.readUtf8()).isEqualTo("client_id=test&scope=openid%20email%20profile%20offline_access&code_challenge=JBP7NwmwWTnwTPLpL30Il_wllvmtC4qeqFXHv-uq6JI&code_challenge_method=S256&redirect_uri=test.okta.com%2Flogin&state=randomGen&activation_token=secret")
        assertThat(request.body?.contentType()).isEqualTo("application/x-www-form-urlencoded".toMediaType())
    }
}
